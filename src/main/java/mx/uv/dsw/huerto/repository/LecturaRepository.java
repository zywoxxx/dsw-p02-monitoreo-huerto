 ...
 * Equivale al CatalogRepository del starter, adaptado al dominio PRxx (huerto).
 * Solo SQL parametrizado; ninguna cadena del usuario se concatena en la consulta.
 */
public class LecturaRepository {

    /** RF-04: lecturas mas recientes con datos del sensor y nivel de alerta si existe. */
    public List<Lectura> findRecientes(int limite) throws SQLException {
        String sql =
            "SELECT l.id, l.sensor_id, s.codigo, t.nombre AS tipo_nombre, t.unidad, z.nombre AS zona, "
        ...
        try (Connection cn = DbConfig.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    l.setTipoNombre(rs.getString("tipo_nombre"));
    ...
    /** RF-02: inserta una lectura dentro de la conexion/transaccion recibida y devuelve su id. */
    ...
    /** RF-05: inserta la alerta asociada a una lectura fuera de umbral. */
    ...
    /** RF-06: alertas mas recientes, primero las no atendidas. */
    public List<Alerta> findAlertas(int limite) throws SQLException {
        String sql =
            "SELECT a.id, a.lectura_id, s.codigo, z.nombre AS zona, l.valor, t.unidad, "
